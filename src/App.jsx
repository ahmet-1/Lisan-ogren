...
function Av({h,dil,sz=64}){
  ...
  return(
    <div style={{width:sz,height:sz,borderRadius:"50%",flexShrink:0,position:"relative",
      ...
      {h.c&&sz>50&&<div style={{position:"absolute",top:-4,right:-4,width:20,height:20,borderRadius:"50%",background:K.gold,border:`2px solid ${K.bg}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,color:"#fff",fontWeight:700}}>★</div>}
    </div>
  );
}
...
function AuthModal({ilkMod,kapat,basari}){
  ...
  const inp=(k,tip="text",yer="")=>(
    <div style={{marginBottom:10}}>
      <input type={tip} value={f[k]} placeholder={yer}
        ...
        style={{width:"100%",padding:"10px 13px",background:K.bg3,border:`1px solid ${h[k]?K.err:K.bdr}`,borderRadius:9,color:K.tx,fontSize:13,outline:"none",boxSizing:"border-box"}}/>
  ...
  const kayit=()=>{
    ...
    if(Object.keys(e).length){setH(e);return;}
    const a=getA();
    if((a.users||[]).find(x=>x.email.toLowerCase()===f.email.toLowerCase())){setH({email:"Bu e-posta zaten kayıtlı"});return;}
    const yeni={id:Date.now(),ad:f.ad,email:f.email,tel:f.tel,tc:f.tc,dogum:f.dogum,sehir:f.sehir,pw:f.sifre,
      plan:"Deneme",durum:"Deneme",dil:"—",tarih:new Date().toLocaleDateString("tr-TR"),
  ...
  const tabS=a=>({flex:1,padding:"10px",border:"none",cursor:"pointer",fontWeight:700,fontSize:13,
    background:a?`linear-gradient(135deg,${K.g2},${K.t2})`:K.bg3,color:a?"#fff":K.tx3,borderRadius:8});
  const btnS={width:"100%",padding:12,background:`linear-gradient(135deg,${K.g2},${K.t2})`,color:"#fff",
    border:"none",borderRadius:10,cursor:"pointer",fontWeight:700,fontSize:14,marginBottom:8};
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.92)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:9000}}>
      <div style={{background:K.card,borderRadius:22,padding:24,width:390,border:`1px solid ${K.bdr3}`,maxHeight:"92vh",overflowY:"auto",boxShadow:"0 24px 64px rgba(0,0,0,0.8)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
          {mod!=="unuttu"&&<div style={{display:"flex",gap:6,flex:1}}>
        ...
        {mod==="giris"&&<>
          ...
          <div style={{color:K.tx3,fontSize:11,marginBottom:3}}>Şifre</div>{inp("sifre","password","••••••••")}
          <div style={{textAlign:"right",marginBottom:12}}><button style={lnk} onClick={()=>{setMod("unuttu");setH({});setMesaj("");}}>Şifremi Unuttum</button></div>
          <button style={btnS} onClick={giris}>Giriş Yap</button>
          <div style={{textAlign:"center",color:K.tx3,fontSize:12}}>Hesabın yok mu? <button style={lnk} onClick={()=>{setMod("kayit");setH({});setTamam(false);}}>Üye Ol</button></div>
        </>}
        ...
        {mod==="kayit"&&(tamam?(
          ...
          <div style={{textAlign:"center",color:K.tx3,fontSize:12}}>Zaten hesabın var mı? <button style={lnk} onClick={()=>{setMod("giris");setH({});}}>Giriş Yap</button></div>
        </>)}
        ...
        {mod==="unuttu"&&(mesaj?(
          ...
          <div style={{color:K.tx3,fontSize:11,marginBottom:3}}>E-posta</div>{inp("email","email","ornek@mail.com")}
          <button style={btnS} onClick={sifre}>Sıfırlama E-postası Gönder</button>
          <div style={{textAlign:"center"}}><button style={lnk} onClick={()=>setMod("giris")}>← Geri Dön</button></div>
        </>)}
      </div>
    </div>
  );
...
function DersEkrani({dilId,hoca,kul,kapat}){
  ...
  useEffect(()=>{
    if(!dilMod) return;
    const hm = kul?.ad?.split(" ")[0]||"";
    const txt = dilMod==="tr"
      ? `Merhaba ${hm}! Ben ${hoca.ad}. ${dil.ad} dersine hoş geldin!\n\nUzmanlığım: ${hoca.uz}\n\nDersimizi Türkçe işleyeceğiz. Mikrofona basarak sesli veya yazarak konuşabilirsin. Hazır mısın?`
      : dilMod==="hedef"
      ? `Merhaba ${hm}! Ben ${hoca.ad}. ${dil.ad} dersine hoş geldin!\n\nUzmanlığım: ${hoca.uz}\n\nDersimizi ${dil.ad} dilinde işleyeceğiz. Hazır mısın?`
  ...
  const sistemPrompt = !dilMod ? "" : dilMod==="tr"
    ? `Sen ${hoca.ad} adlı uzman bir AI dil öğretmenisin. ${hoca.yer} kökenlisin. ${dil.ad} öğretiyorsun. Uzmanlık: ${hoca.uz}.

GÖREVIN:
- Samimi, sıcak ve sabırlı bir hoca gibi konuş. Tıpkı telefonda konuşur gibi doğal ol.
- SADECE TÜRKÇE yanıt ver.
- Öğrencinin yazdığı veya söylediği her şeyi dikkatle analiz et.
- Telaffuz hatalarını, yazım yanlışlarını ve gramer hatalarını MUTLAKA düzelt.
- Düzeltmeyi nazikçe yap: önce doğrusunu söyle, sonra kısa açıkla.
- Örnek: "Aslında doğru söyleniş 'Bismillahirrahmanirrahim' şeklinde. Senin söyleyişinde son hece biraz farklıydı."
- Her derste öğrenciye yeni bir şey öğret.
...
GÖREVIN:
- Samimi, sıcak ve sabırlı bir hoca gibi konuş. Tıpkı telefonda konuşur gibi doğal ol.
- ${dil.ad} dilinde yanıt ver.
- Öğrencinin yazdığı veya söylediği her şeyi dikkatle analiz et.
- Telaffuz hatalarını, yazım yanlışlarını ve gramer hatalarını MUTLAKA düzelt.
- Düzeltmeyi nazikçe yap: önce doğrusunu göster, sonra kısa açıkla.
- Her derste öğrenciye yeni bir şey öğret.
- Maksimum 3 paragraf yaz.`
    : `Sen ${hoca.ad} adlı uzman bir AI dil öğretmenisin. ${hoca.yer} kökenlisin. ${dil.ad} öğretiyorsun. Uzmanlık: ${hoca.uz}.
...
GÖREVIN:
- Samimi, sıcak ve sabırlı bir hoca gibi konuş. Tıpkı telefonda konuşur gibi doğal ol.
- Hem Türkçe hem ${dil.ad} kullanarak yanıt ver. Açıklamaları Türkçe yap, örnekleri ${dil.ad} dilinde ver.
- Öğrencinin yazdığı veya söylediği her şeyi dikkatle analiz et.
- Telaffuz hatalarını, yazım yanlışlarını ve gramer hatalarını MUTLAKA düzelt.
- Düzeltmeyi nazikçe yap: önce doğrusunu göster, Türkçe açıkla.
- Her derste öğrenciye yeni bir şey öğret.
- Maksimum 3 paragraf yaz.`;
    ...
    return(
      <div style={{position:"fixed",inset:0,background:K.bg,display:"flex",alignItems:"center",justifyContent:"center",zIndex:8000}}>
        <div style={{background:K.card,borderRadius:22,padding:36,width:400,border:"1px solid "+K.bdr3,textAlign:"center",boxShadow:"0 24px 64px rgba(0,0,0,0.8)"}}>
          ...
          {[
            {id:"tr",    b:"🇹🇷 Türkçe",          a:"Hoca Türkçe konuşur ve açıklar"},
            ...
            <div key={s.id} onClick={()=>setDilMod(s.id)}
              style={{background:K.bg3,borderRadius:12,padding:"14px 18px",marginBottom:10,cursor:"pointer",
                border:"1px solid "+K.bdr,textAlign:"left"}}
  ...
  const mikBasla=()=>{
    ...
    try{
      ...
      r.onerror=e=>{setMikr(false);setMikErr(e.error==="not-allowed"?"Mikrofon izni reddedildi.":"Ses algılanamadı.");setTimeout(()=>setMikErr(""),4000);};
  ...
  return(
    <div style={{position:"fixed",inset:0,background:K.bg,display:"flex",flexDirection:"column",zIndex:8000}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:12,padding:"10px 16px",background:"linear-gradient(135deg,"+dil.renk+"ee,"+dil.renk+"99)",borderBottom:"2px solid "+dil.vurgu}}>
        ...
        <div style={{background:"rgba(0,0,0,0.3)",borderRadius:8,padding:"4px 10px",fontSize:11,color:"#fff",cursor:"pointer"}} onClick={()=>{setDilMod(null);setMsgs([]);}}>{dilLabel} ↺</div>
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
        <div style={{width:190,background:K.bg2,borderRight:"1px solid "+K.bdr,padding:10,display:"flex",flexDirection:"column",gap:10,overflowY:"auto"}}>
          ...
          <div style={{background:K.card,borderRadius:10,padding:12,textAlign:"center"}}>
            ...
            <button onClick={()=>{setDilMod(null);setMsgs([]);}} style={{width:"100%",marginTop:8,padding:"7px",borderRadius:7,background:"rgba(46,125,50,0.12)",color:K.gL,border:"1px solid "+K.g2+"44",cursor:"pointer",fontSize:11,fontWeight:600}}>Dil Değiştir</button>
          </div>
        </div>
            ...
            {msgs.map((m,i)=>(
              <div key={i} style={{display:"flex",justifyContent:m.r==="user"?"flex-end":"flex-start",gap:8,alignItems:"flex-start"}}>
                ...
                <div style={{maxWidth:"70%"}}>
                  <div style={{fontSize:10,color:K.tx4,marginBottom:3,textAlign:m.r==="user"?"right":"left"}}>{m.r==="user"?"Sen":"🤖 "+hoca.ad.split(" ")[0]}</div>
          ...
          <div style={{padding:12,borderTop:"1px solid "+K.bdr,background:K.bg2}}>
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              ...
              <input value={yazi} onChange={e=>setYazi(e.target.value)}
                ...
                placeholder={mikr?"Dinliyorum...":"Mesaj yaz veya 🎤 bas konuş..."}
            ...
            <div style={{textAlign:"center",color:K.tx4,fontSize:10,marginTop:5}}>🎤 Bas konuş bırak gönder • Her konuşma ekrana yazılır</div>
          </div>
        </div>
      </div>
    </div>
...
function AdminPanel({kapat}){
  ...
  const onayOde=id=>{
    ...
    kaydet({...cfg,pays:ode.map(x=>x.id===id?{...x,d:"ok"}:x),
  ...
  const SEKMELER=[["dash","📊","Dashboard"],["kul","👥","Kullanıcılar"],["ode","💳","Ödemeler"],["ders","📡","Aktif Dersler"],["hed","🎁","Hediye Ver"],["bil","🔔","Bildirimler"],["set","⚙️","Ayarlar"]];
  ...
  return(
    <div style={{position:"fixed",inset:0,background:K.bg,zIndex:7000,display:"flex"}}>
      <div style={{width:210,background:K.bg2,borderRight:`1px solid ${K.bdr}`,display:"flex",flexDirection:"column",padding:14,gap:3}}>
        ...
        <div style={{flex:1}}/>
        <button onClick={kapat} style={{padding:"10px 12px",borderRadius:9,border:`1px solid ${K.bdr}`,background:"transparent",color:K.tx4,cursor:"pointer",fontSize:12}}>← Uygulamaya Dön</button>
      </div>
        ...
        {sekme==="dash"&&<>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:16}}>
            {[{l:"Toplam Kullanıcı",v:toplam,c:K.gL},{l:"Aktif Abonelik",v:aktif,c:K.tL},{l:"Deneme Süreci",v:deneme,c:K.warn},
              {l:"Bekleyen Ödeme",v:bekl,c:K.errL},{l:"Toplam Gelir",v:`₺${gelir.toLocaleString()}`,c:K.warn},{l:"Toplam Hoca",v:60,c:K.gL}].map(s=>(
                ...
                {["7 Gün","1 Ay","3 Ay","Yıllık","Sınırsız"].map(g=>(
                  <div key={g} onClick={()=>setHT(g)}
                    style={{padding:"10px 14px",borderRadius:9,background:hT===g?"rgba(46,125,50,0.2)":K.bg3,
                      border:`1px solid ${hT===g?K.g3:K.bdr}`,color:hT===g?K.gL:K.tx2,cursor:"pointer",marginBottom:7,fontSize:12,fontWeight:hT===g?700:400}}>
                    🎁 {g} Ücretsiz
                  </div>
                ))}
        ...
        {sekme==="bil"&&<>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            {[{t:"Premium Teşvik",m:"5 günlük denemeniz bitiyor! Premium'a geçin."},
              ...
              {t:"Ders Hatırlatma",m:"Bugün ders yapmadınız. Hocanız bekliyor."}].map(n=>(
              <div key={n.t} style={{...kd,marginBottom:0}}>
                ...
                <button onClick={()=>alert("Bildirim gönderildi!")} style={{width:"100%",padding:"7px",borderRadius:7,background:"rgba(46,125,50,0.12)",color:K.gL,border:`1px solid ${K.g2}44`,cursor:"pointer",fontSize:11,fontWeight:600}}>Tüm Kullanıcılara Gönder</button>
              </div>
            ))}
          </div>
        </>}
...
export default function App(){
  ...
  const admGiris=()=>{
    const a=getA();
    if(adSifre===a.pw){setAdGir(true);DB.s("adGir",true);setAdAcik(true);setAdModal(false);setAdSifre("");setAdHata("");}
  ...
  if(adAcik||adGir)return <AdminPanel kapat={()=>setAdAcik(false)}/>;
    ...
    <div style={{minHeight:"100vh",background:`linear-gradient(170deg,${K.bg},${K.bg2} 50%,${K.bg})`,fontFamily:"'Segoe UI',system-ui,sans-serif"}}>
      ...
      <nav style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"13px 22px",
        ...
        <div style={{display:"flex",gap:7,alignItems:"center"}}>
          {kul?(
            <>
              <div style={{background:"rgba(46,125,50,0.12)",borderRadius:8,padding:"6px 13px",fontSize:12,color:K.gL,fontWeight:600,border:`1px solid ${K.g2}33`}}>
                👤 {kul.ad.split(" ")[0]}<span style={{color:kul.durum==="Aktif"?K.gL:K.warn,fontSize:10,marginLeft:5}}>{kul.durum}</span>
              </div>
          ...
          <button onClick={()=>{setAdModal(true);setAdHata("");setAdSifre("");}} style={{padding:"6px 9px",borderRadius:8,border:`1px solid ${K.bdr}`,background:"transparent",color:K.tx4,cursor:"pointer",fontSize:10}}>⚙</button>
        </div>
      </nav>
        ...
        <div style={{animation:"gir 0.5s ease"}}>
          <div style={{textAlign:"center",padding:"68px 22px 42px"}}>
            <div style={{display:"inline-flex",alignItems:"center",gap:7,background:"rgba(46,125,50,0.1)",
              border:"1px solid rgba(46,125,50,0.25)",borderRadius:20,padding:"5px 16px",fontSize:11,color:K.gL,marginBottom:22,fontWeight:600}}>
          ...
          <div style={{display:"flex",justifyContent:"center",gap:18,padding:"0 22px 36px",flexWrap:"wrap",alignItems:"flex-end"}}>
            {[{ad:"Şeyh Ahmed Al-Ghamdi",p:4.9,n:1240,c:false,uz:"Tecvid",dil:DILLER[0],a:0},
              ...
              {ad:"Dr. Natasha Ivanova",p:4.9,n:2000,c:false,uz:"Русский",dil:DILLER[8],a:2},
              ...
              {ad:"Prof. Klaus Weber",p:4.9,n:1800,c:false,uz:"Deutsch",dil:DILLER[3],a:2}].map((h,i)=>(
          ...
          <div style={{display:"flex",gap:12,padding:"0 22px 36px",justifyContent:"center",flexWrap:"wrap"}}>
            {[{t:"🎤 Sesli Konuşma",d:"Mikrofona bas, hocanla sesli konuş"},
              ...
              {t:"👶 Çocuk Modu",d:"Her dilde özel çocuk hocaları"}].map(f=>(
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.92)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:9000}}>
          <div style={{background:K.card,borderRadius:18,padding:26,width:300,border:`1px solid ${K.bdr3}`,boxShadow:"0 24px 64px rgba(0,0,0,0.8)"}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:16}}>
              ...
              <button onClick={()=>{setAdModal(false);setAdHata("");setAdSifre("");}} style={{background:"none",border:"none",color:K.tx4,fontSize:18,cursor:"pointer"}}>✕</button>
            </div>
