...
const DILLER=[
  ...
  {id:"chinese",ad:"Çince",           yerel:"中文",          bayrak:"🇨🇳",renk:"#2a0a0a",vurgu:"#fdd835",acik:"Mandarin & HSK",              mic:"zh-CN",mods:["Pinyin","Hanzi","Konuşma","HSK"]},
  {id:"turkish",ad:"Türkçe",          yerel:"Türkçe",       bayrak:"🇹🇷",renk:"#2a0a0a",vurgu:"#ecf0f1",acik:"Ana dil & Yabancılara",       mic:"tr-TR",mods:["Dilbilgisi","Konuşma","Yazma","TÖMER"]},
...
function Av({h,dil,sz=64}){
  ...
  return(
    <div style={{width:sz,height:sz,borderRadius:"50%",flexShrink:0,position:"relative",
      ...
      {h.c&&sz>50&&<div style={{position:"absolute",top:-4,right:-4,width:20,height:20,borderRadius:"50%",
...
function AuthModal({ilkMod,kapat,basari}){
  ...
  const kayit=()=>{
    ...
    const yeni={id:Date.now(),ad:f.ad,email:f.email,tel:f.tel,tc:f.tc,
      ...
      dil:"—",tarih:new Date().toLocaleDateString("tr-TR"),
  ...
  const btnS={width:"100%",padding:12,background:"linear-gradient(135deg,"+K.g2+","+K.t2+")",
      ...
      <div style={{background:K.card,borderRadius:22,padding:24,width:390,
        ...
        {mod==="giris"&&(
          <>
            ...
            <button style={btnS} onClick={giris}>Giriş Yap</button>
            <div style={{textAlign:"center",color:K.tx3,fontSize:12}}>
              Hesabın yok mu?{" "}
              <button style={lnk} onClick={()=>{setMod("kayit");setH({});setTamam(false);}}>Üye Ol</button>
            </div>
          </>
        )}
        ...
        {mod==="kayit"&&(
          tamam?(
            <div style={{textAlign:"center",padding:"16px 0"}}>
              ...
              <button style={btnS} onClick={kapat}>Derse Başla →</button>
            ...
            <>
              ...
              <div style={{background:K.bg3,borderRadius:9,padding:11,marginBottom:12,border:"1px solid "+K.bdr}}>
                <label style={{display:"flex",gap:9,cursor:"pointer",alignItems:"flex-start"}}>
                  <input type="checkbox" checked={f.onay}
                    onChange={e=>setF(p=>({...p,onay:e.target.checked}))}
              ...
              <button style={btnS} onClick={kayit}>Kayıt Ol →</button>
              <div style={{textAlign:"center",color:K.tx3,fontSize:12}}>
                Zaten hesabın var mı?{" "}
                <button style={lnk} onClick={()=>{setMod("giris");setH({});}}>Giriş Yap</button>
              </div>
            </>
          )
        )}
        ...
        {mod==="unuttu"&&(
          mesaj?(
            <div style={{textAlign:"center",padding:"16px 0"}}>
              ...
              <button style={btnS} onClick={()=>setMod("giris")}>Giriş Yap</button>
            </div>
          ):(
            <>
              ...
              <div style={{color:K.tx3,fontSize:11,marginBottom:3}}>E-posta</div>
              {inp("email","email","ornek@mail.com")}
              <button style={btnS} onClick={sifreSifirla}>Sıfırlama E-postası Gönder</button>
...
function DersEkrani({dilId,hoca,kul,kapat}){
  ...
  useEffect(()=>{
    ...
    const txt=dilMod==="tr"
      ?`Merhaba ${ad}! Ben ${hoca.ad}.\n\nUzmanlığım: ${hoca.uz}\n\nDersimizi Türkçe işleyeceğiz. Hazır mısın?`
      :dilMod==="hedef"
      ?`Merhaba ${ad}! Ben ${hoca.ad}.\n\nUzmanlığım: ${hoca.uz}\n\n${dil.ad} dilinde ders yapacağız. Hazır mısın?`
      :`Merhaba ${ad}! Ben ${hoca.ad}.\n\nUzmanlığım: ${hoca.uz}\n\nHem Türkçe hem ${dil.ad} kullanarak ders işleyeceğiz. Hazır mısın?`;
    setMsgs([{r:"ai",t:txt}]);
  },[dilMod]);

  useEffect(()=>{sonRef.current?.scrollIntoView({behavior:"smooth"});},[msgs]);

  const prompt=!dilMod?"":dilMod==="tr"
    ?`Sen ${hoca.ad} adlı uzman bir AI dil öğretmenisin. ${hoca.yer} kökenlisin. ${dil.ad} öğretiyorsun. Uzmanlık: ${hoca.uz}. SADECE TÜRKÇE yanıt ver. Samimi ve sıcak bir hoca gibi konuş. Öğrencinin yazdığı veya söylediği her şeyi analiz et, telaffuz ve yazım hatalarını nazikçe düzelt. Önce doğrusunu söyle sonra kısaca açıkla. Maksimum 3 paragraf.`
    :dilMod==="hedef"
    ?`Sen ${hoca.ad} adlı uzman bir AI dil öğretmenisin. ${hoca.yer} kökenlisin. ${dil.ad} öğretiyorsun. Uzmanlık: ${hoca.uz}. SADECE ${dil.ad} dilinde yanıt ver. Samimi ve sıcak bir hoca gibi konuş. Öğrencinin hatalarını nazikçe düzelt. Maksimum 3 paragraf.`
    :`Sen ${hoca.ad} adlı uzman bir AI dil öğretmenisin. ${hoca.yer} kökenlisin. ${dil.ad} öğretiyorsun. Uzmanlık: ${hoca.uz}. Hem Türkçe hem ${dil.ad} kullan. Açıklamaları Türkçe yap, örnekleri ${dil.ad} dilinde ver. Öğrencinin hatalarını nazikçe düzelt. Maksimum 3 paragraf.`;

  if(!dilMod){
    return(
      <div style={{position:"fixed",inset:0,background:K.bg,display:"flex",
        alignItems:"center",justifyContent:"center",zIndex:8000}}>
        <div style={{background:K.card,borderRadius:22,padding:36,width:400,
          border:"1px solid "+K.bdr3,textAlign:"center",boxShadow:"0 24px 64px rgba(0,0,0,0.8)"}}>
          <div style={{display:"flex",justifyContent:"center",marginBottom:16}}>
            <Av h={hoca} dil={dil} sz={80}/>
          </div>
          ...
          <button onClick={kapat}
            style={{marginTop:10,padding:"9px 24px",background:"transparent",
              color:K.tx4,border:"1px solid "+K.bdr,borderRadius:9,cursor:"pointer",fontSize:13}}>
            ← Geri
          </button>
        </div>
      </div>
    ...
    try{
      const res=await fetch("https://api.anthropic.com/v1/messages",{
        ...
        body:JSON.stringify({
          ...
          system:prompt,
          messages:[...msgs.filter(m=>m.r).map(m=>({role:m.r==="ai"?"assistant":"user",content:m.t})),
  ...
  return(
    <div style={{position:"fixed",inset:0,background:K.bg,display:"flex",flexDirection:"column",zIndex:8000}}>
      ...
      <div style={{background:"rgba(27,94,32,0.2)",padding:"4px 16px",fontSize:11,
        color:K.gL,textAlign:"center",borderBottom:"1px solid "+K.g2+"44"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:12,padding:"10px 16px",
        ...
        <div style={{background:"rgba(0,0,0,0.3)",borderRadius:8,padding:"4px 10px",
        ...
        <button onClick={kapat}
          style={{background:"rgba(255,255,255,0.12)",border:"none",color:"#fff",
            borderRadius:8,padding:"8px 14px",cursor:"pointer",fontWeight:700}}>✕ Çıkış</button>
      </div>

      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
        <div style={{width:190,background:K.bg2,borderRight:"1px solid "+K.bdr,padding:10,
          display:"flex",flexDirection:"column",gap:10,overflowY:"auto"}}>
          <div style={{background:K.card,borderRadius:10,padding:12,
            border:"1px solid "+K.bdr2,textAlign:"center"}}>
          ...
          {mikErr&&<div style={{background:"rgba(198,40,40,0.12)",borderRadius:8,padding:10,
            color:K.errL,fontSize:11,border:"1px solid "+K.err+"44"}}>{mikErr}</div>}
          <div style={{background:K.card,borderRadius:10,padding:12}}>
            <div style={{fontSize:9,color:K.tx4,marginBottom:8,fontWeight:700,letterSpacing:1}}>MODÜLLER</div>
            {dil.mods.map(m=><div key={m} style={{padding:"6px 10px",borderRadius:7,marginBottom:4,
              background:K.bg3,color:K.tx2,fontSize:11,borderLeft:"3px solid "+dil.vurgu+"55"}}>{m}</div>)}
          </div>
          <div style={{background:K.card,borderRadius:10,padding:12,textAlign:"center"}}>
            ...
            <button onClick={()=>{setDilMod(null);setMsgs([]);}}
              style={{width:"100%",marginTop:8,padding:"7px",borderRadius:7,
                background:"rgba(46,125,50,0.12)",color:K.gL,
                border:"1px solid "+K.g2+"44",cursor:"pointer",fontSize:11,fontWeight:600}}>
              Dil Değiştir
            </button>
          </div>
        </div>
          ...
          <div style={{flex:1,overflowY:"auto",padding:16,display:"flex",flexDirection:"column",gap:12}}>
            {msgs.map((m,i)=>(
              <div key={i} style={{display:"flex",justifyContent:m.r==="user"?"flex-end":"flex-start",
                ...
                <div style={{maxWidth:"70%"}}>
                  <div style={{fontSize:10,color:K.tx4,marginBottom:3,
                    textAlign:m.r==="user"?"right":"left"}}>
                    {m.r==="user"?"Sen":"🤖 "+hoca.ad.split(" ")[0]}
                  </div>
                  <div style={{padding:"11px 14px",borderRadius:16,color:K.tx,fontSize:13,
            ...
            {yukl&&(
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                ...
                <div style={{background:K.card,borderRadius:16,padding:"10px 16px",
                  border:"1px solid "+K.bdr,display:"flex",gap:4,alignItems:"center"}}>
                  {[0,1,2].map(i=><div key={i} className="nk"
                    style={{"--d":i*0.18+"s",width:7,height:7,borderRadius:"50%",background:K.gL}}/>)}
                </div>
              </div>
            )}
            <div ref={sonRef}/>
          ...
          <div style={{padding:12,borderTop:"1px solid "+K.bdr,background:K.bg2}}>
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              ...
              <input value={yazi} onChange={e=>setYazi(e.target.value)}
                ...
                style={{flex:1,background:K.bg3,border:"1px solid "+K.bdr,
...
function AdminPanel({kapat}){
  ...
  const SEKMELER=[["dash","📊","Dashboard"],["kul","👥","Kullanıcılar"],
    ["ode","💳","Ödemeler"],["ders","📡","Aktif Dersler"],
    ["hed","🎁","Hediye Ver"],["bil","🔔","Bildirimler"],["set","⚙️","Ayarlar"]];
  ...
  return(
    <div style={{position:"fixed",inset:0,background:K.bg,zIndex:7000,display:"flex"}}>
      ...
      <div style={{width:210,background:K.bg2,borderRight:"1px solid "+K.bdr,
        display:"flex",flexDirection:"column",padding:14,gap:3}}>
        {/* Logo - tıklanınca uygulamaya dön */}
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:16,
          paddingBottom:14,borderBottom:"1px solid "+K.bdr,cursor:"pointer"}}
          onClick={kapat}>
          <div style={{width:34,height:34,borderRadius:9,
            background:"linear-gradient(135deg,"+K.g4+","+K.t3+")",
            display:"flex",alignItems:"center",justifyContent:"center",
            color:"#fff",fontWeight:900,fontSize:17}}>L</div>
          <span style={{fontWeight:900,color:K.tx,fontSize:15}}>
            Lingua<span style={{color:K.gL}}>AI</span>
          </span>
        </div>
        {SEKMELER.map(([id,ic,lb])=>(
          <button key={id} onClick={()=>setSekme(id)}
            style={{display:"flex",alignItems:"center",gap:8,padding:"10px 12px",
              ...
              color:sekme===id?K.gL:K.tx4,cursor:"pointer",fontSize:12,
              textAlign:"left",fontWeight:sekme===id?700:400,
      ...
      <div style={{flex:1,overflowY:"auto",padding:22}}>
        {sekme==="dash"&&<>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:16}}>
            {[{l:"Toplam Kullanıcı",v:toplam,c:K.gL},{l:"Aktif Abonelik",v:aktif,c:K.tL},
              ...
              {l:"Toplam Gelir",v:"₺"+gelir.toLocaleString(),c:K.warn},{l:"Toplam Hoca",v:60,c:K.gL}
            ].map(s=>(
        ...
        {sekme==="kul"&&<>
          ...
          {kul.length===0?(
            ...
            <div style={{...kd,padding:0,overflow:"hidden"}}>
              <div style={{display:"grid",gridTemplateColumns:"2fr 1.5fr 1fr 1fr 0.8fr",
                padding:"9px 14px",background:K.bg3,fontSize:9,color:K.tx4,fontWeight:700,letterSpacing:0.5}}>
              ...
              {kul.map(u=>(
                <div key={u.id} style={{display:"grid",gridTemplateColumns:"2fr 1.5fr 1fr 1fr 0.8fr",
                  ...
                  <div style={{display:"inline-block",borderRadius:5,padding:"2px 7px",fontSize:10,fontWeight:600,
                    ...
                    color:u.durum==="Aktif"?K.gL:u.durum==="Deneme"?K.warn:K.errL}}>
                    {u.durum}
                  </div>
        ...
        {sekme==="ode"&&<>
          ...
          {cfg.iban&&<div style={kd}>
            <div style={{color:K.tx2,fontWeight:700,marginBottom:10}}>Aktif IBAN</div>
            <div style={{color:K.tx3,fontSize:13,lineHeight:2.2}}>
              {cfg.acName}<br/>
              <strong style={{color:K.gL,fontFamily:"monospace",fontSize:14}}>{cfg.iban}</strong><br/>
              {cfg.bank}
            </div>
          </div>}
          ...
          {ode.filter(o=>o.d==="ok").length===0?(
            ...
            <div key={o.id} style={{...kd,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              ...
              <div style={{display:"flex",gap:10,alignItems:"center"}}>
                ...
                <div style={{background:"rgba(46,125,50,0.15)",color:K.gL,borderRadius:5,
                  padding:"2px 8px",fontSize:10,fontWeight:600}}>✓ Onaylı</div>
              </div>
            </div>
          ))}
        </>}
        ...
        {sekme==="ders"&&<>
          ...
          {kul.filter(u=>u.durum==="Aktif").length===0?(
            ...
            <div key={u.id} style={{...kd,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              ...
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                <div style={{background:"rgba(46,125,50,0.12)",color:K.gL,
                  borderRadius:6,padding:"3px 10px",fontSize:11,fontWeight:700}}>AKTİF</div>
                <button onClick={()=>setIzle(u)}
                  style={{padding:"7px 12px",borderRadius:7,background:K.bg3,
                    color:K.tL,border:"1px solid "+K.bdr2,cursor:"pointer",fontSize:11,fontWeight:600}}>
                  👁 İzle
                </button>
              </div>
            </div>
              ...
              <div style={{background:K.card,borderRadius:18,padding:26,width:360,border:"1px solid "+K.bdr3}}>
                ...
                <div style={{background:"rgba(249,168,37,0.08)",borderRadius:10,padding:12,
                  ...
                  <div style={{color:K.tx4,fontSize:11,lineHeight:1.8}}>
                    • Kullanıcı sizin katıldığınızı görmez<br/>• Kayıt yapılmaz<br/>• Yalnızca hizmet kalitesi denetimi
                  </div>
                </div>
                <div style={{display:"flex",gap:10}}>
                  <button onClick={()=>setIzle(null)}
                ...
                {["7 Gün","1 Ay","3 Ay","Yıllık","Sınırsız"].map(g=>(
                  <div key={g} onClick={()=>setHT(g)}
                    style={{padding:"10px 14px",borderRadius:9,
                      ...
                      color:hT===g?K.gL:K.tx2,cursor:"pointer",marginBottom:7,
                      fontSize:12,fontWeight:hT===g?700:400}}>
                    {"🎁 "+g+" Ücretsiz"}
                  </div>
                ))}
                <button onClick={hediye} style={{...bG,width:"100%",padding:"12px",marginTop:4,fontSize:14}}>
                  Hediye Gönder
                </button>
              </>
            )}
        ...
        {sekme==="set"&&<>
          ...
          <div style={kd}>
            ...
            <input value={cfg.email||""} onChange={e=>setCfg(s=>({...s,email:e.target.value}))}
            ...
            <input value={cfg.contactEmail||""} onChange={e=>setCfg(s=>({...s,contactEmail:e.target.value}))}
          ...
          <div style={kd}>
            ...
            <input type="password" value={p1} onChange={e=>setP1(e.target.value)}
              placeholder="Yeni şifre" style={gI}/>
            <input type="password" value={p2} onChange={e=>setP2(e.target.value)}
              placeholder="Tekrar girin" style={gI}/>
            {pMsg&&<div style={{color:pMsg.startsWith("✅")?K.gL:K.errL,fontSize:12,marginBottom:10}}>{pMsg}</div>}
            <button onClick={sifreDegis}
              style={{padding:"9px 18px",background:"rgba(46,125,50,0.15)",color:K.gL,
                border:"1px solid "+K.g2+"55",borderRadius:9,cursor:"pointer",fontWeight:700,fontSize:12}}>
              Şifreyi Güncelle
            </button>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:14}}>
            <button onClick={()=>kaydet(cfg)} style={{...bG,padding:"13px 28px",fontSize:15}}>
              💾 Tüm Ayarları Kaydet
            </button>
...
export default function App(){
  ...
  if(ders)return <DersEkrani dilId={ders.dil} hoca={ders.hoca}
    kul={ders.kul||kul} kapat={()=>setDers(null)}/>;

  const bP={padding:"13px 28px",background:"linear-gradient(135deg,"+K.g2+","+K.t2+")",
  ...
  const gI2={width:"100%",padding:"11px 13px",background:K.bg3,
    border:"1px solid "+K.bdr,borderRadius:9,color:K.tx,fontSize:13,
    outline:"none",boxSizing:"border-box"};

  return(
    <div style={{minHeight:"100vh",
      background:"linear-gradient(170deg,"+K.bg+","+K.bg2+" 50%,"+K.bg+")",
      ...
      <nav style={{display:"flex",alignItems:"center",justifyContent:"space-between",
        ...
        background:"rgba(7,21,16,0.97)",position:"sticky",top:0,zIndex:100,
        backdropFilter:"blur(20px)"}}>
        {/* Logo - her zaman ana sayfaya götürür */}
          ...
          {[["ana","Ana Sayfa"],["diller","Diller"],["fiyatlar","Fiyatlar"],["iletisim","İletişim"]].map(([s,l])=>(
            <button key={s} onClick={()=>git(s)}
              style={{padding:"7px 13px",borderRadius:8,border:"none",cursor:"pointer",
                fontSize:12,fontWeight:sayfa===s?700:400,
        ...
        <div style={{display:"flex",gap:7,alignItems:"center"}}>
          {kul?(
            <>
              <div style={{background:"rgba(46,125,50,0.12)",borderRadius:8,
                padding:"6px 13px",fontSize:12,color:K.gL,fontWeight:600,
                border:"1px solid "+K.g2+"33"}}>
                👤 {kul.ad.split(" ")[0]}
                <span style={{color:kul.durum==="Aktif"?K.gL:K.warn,fontSize:10,marginLeft:5}}>
                  {kul.durum}
                </span>
              </div>
              <button onClick={kulCikis}
                style={{padding:"6px 11px",borderRadius:8,border:"1px solid "+K.bdr,
                  background:"transparent",color:K.tx4,cursor:"pointer",fontSize:11}}>
                Çıkış
              </button>
            </>
          ):(
            <>
              <button onClick={()=>{setAuthMod("giris");setAuthAcik(true);}}
                style={{padding:"7px 14px",borderRadius:8,border:"1px solid "+K.bdr,
                  background:"transparent",color:K.tx2,cursor:"pointer",fontSize:12,fontWeight:600}}>
                Giriş Yap
              </button>
          ...
          <button onClick={()=>{setAdModal(true);setAdHata("");setAdSifre("");}}
            style={{padding:"6px 9px",borderRadius:8,border:"1px solid "+K.bdr,
              background:"transparent",color:K.tx4,cursor:"pointer",fontSize:10}}>⚙</button>
        </div>
      </nav>
      ...
      {sayfa==="ana"&&(
        <div style={{animation:"gir 0.5s ease"}}>
          <div style={{textAlign:"center",padding:"68px 22px 42px"}}>
            ...
            <h1 style={{fontSize:48,fontWeight:900,lineHeight:1.08,
            ...
            <p style={{fontSize:15,color:K.tx3,maxWidth:440,margin:"0 auto 30px",lineHeight:1.8}}>
              Yaz veya mikrofona bas, AI hocanla birebir ders yap.<br/>
              Kameralı özellik yakında güncellemeyle geliyor!
            </p>
            <div style={{display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap"}}>
              <button style={bP} onClick={()=>{
                if(kul)git("diller");
                else{setAuthMod("kayit");setAuthAcik(true);}
          ...
          <div style={{display:"flex",justifyContent:"center",gap:18,
            padding:"0 22px 36px",flexWrap:"wrap",alignItems:"flex-end"}}>
            {[
              ...
              {ad:"Lin Mei",p:4.9,n:2400,c:false,uz:"中文",dil:DILLER[6],a:1},
              {ad:"Prof. Klaus Weber",p:4.9,n:1800,c:false,uz:"Deutsch",dil:DILLER[3],a:2},
            ].map((h,i)=>(
              <div key={i} style={{textAlign:"center",
                animation:"y"+h.a+" "+(2.8+i*0.25)+"s ease-in-out infinite",
                cursor:"pointer"}} onClick={()=>git("diller")}>
          ...
          <div style={{display:"flex",gap:12,padding:"0 22px 36px",
              ...
              <div key={d.id} onClick={()=>setDilSec(d)}
                ...
                <div style={{padding:14}}>
                  ...
                  <div style={{display:"flex",gap:3,flexWrap:"wrap",marginBottom:12}}>
                    {d.mods.map(m=><span key={m} style={{background:K.bg3,
                      border:"1px solid "+d.vurgu+"22",borderRadius:4,
                      padding:"2px 6px",fontSize:10,color:K.tx4}}>{m}</span>)}
                  </div>
                  <div style={{display:"flex",gap:4,alignItems:"center"}}>
                    {HOCALAR[d.id].filter(h=>!h.c).map(h=><Av key={h.id} h={h} dil={d} sz={26}/>)}
                    <div style={{background:K.bg3,borderRadius:5,padding:"2px 7px",
                      fontSize:9,color:K.tL,fontWeight:600}}>+2 Çocuk</div>
                  </div>
                </div>
              </div>
            ))}
      ...
      {sayfa==="diller"&&dilSec&&(
        <div style={{padding:"26px 22px"}}>
          ...
          <div style={{textAlign:"center",marginBottom:22}}>
            ...
            <h2 style={{fontSize:22,fontWeight:800,marginBottom:5,color:K.tx}}>
              {dilSec.ad+" — Hocanı Seç"}
            </h2>
          </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(210px,1fr))",
            ...
            {HOCALAR[dilSec.id].filter(h=>h.c===cocuk).map(h=>(
              <div key={h.id}
                style={{background:K.card,borderRadius:16,padding:18,
                  border:"1px solid "+K.bdr,textAlign:"center",cursor:"pointer",transition:"all 0.2s"}}
                ...
                {h.c&&<div style={{background:"rgba(249,168,37,0.12)",color:K.warn,
                  borderRadius:6,padding:"2px 8px",fontSize:10,fontWeight:700,
                  marginBottom:8,display:"inline-block",border:"1px solid "+K.warn+"33"}}>
                  👶 Çocuklara Özel
                </div>}
                ...
                <div style={{background:K.bg3,borderRadius:7,padding:"3px 9px",
                  fontSize:11,color:K.tx2,marginBottom:10,display:"inline-block"}}>{h.uz}</div>
                ...
                <button style={{width:"100%",padding:"9px",borderRadius:9,
                  background:"linear-gradient(135deg,"+K.g2+","+K.t2+")",
                  color:"#fff",border:"none",cursor:"pointer",fontWeight:700,fontSize:12}}>
                  🎤 Derse Başla
                </button>
        ...
        <div style={{padding:"50px 22px",textAlign:"center"}}>
          ...
          <div style={{display:"flex",gap:16,justifyContent:"center",flexWrap:"wrap"}}>
            {[
              ...
              <div key={p.id}
                ...
                {p.hl&&<div style={{position:"absolute",top:-12,left:"50%",
                  transform:"translateX(-50%)",
                  background:"linear-gradient(135deg,"+K.g3+","+K.t3+")",
                  color:"#fff",borderRadius:18,padding:"3px 14px",
                  fontSize:10,fontWeight:700,whiteSpace:"nowrap"}}>⭐ EN POPÜLER</div>}
                ...
                }} style={{width:"100%",marginTop:18,padding:11,borderRadius:10,
          ...
          {adm.iban&&(
            <div style={{marginTop:34,background:K.card,borderRadius:14,padding:22,
              ...
              <div style={{background:"rgba(46,125,50,0.08)",borderRadius:8,
                padding:10,marginTop:10,border:"1px solid "+K.g2+"33"}}>
                <div style={{color:K.tx4,fontSize:11,lineHeight:1.7}}>
                  Açıklama kısmına e-postanızı yazın. Onaydan sonra aktifleşir (max 2 saat).
                </div>
              </div>
            </div>
          )}
          ...
          <div style={{background:K.card,borderRadius:16,padding:24,border:"1px solid "+K.bdr}}>
            ...
            <div style={{borderTop:"1px solid "+K.bdr,paddingTop:18}}>
              ...
              <textarea placeholder="Mesajınız..." rows={4}
                style={{...gI2,resize:"vertical",marginBottom:14}}/>
      ...
      {odePlan&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.92)",
          ...
          <div style={{background:K.card,borderRadius:20,padding:26,width:390,
            ...
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:16}}>
              ...
              <button onClick={()=>setOdePlan(null)}
            ...
            {adm.iban?(
              <div style={{background:K.bg3,borderRadius:11,padding:15,marginBottom:14,border:"1px solid "+K.bdr}}>
                ...
                <div style={{background:"rgba(46,125,50,0.08)",borderRadius:7,
                  padding:9,marginTop:9,border:"1px solid "+K.g2+"33"}}>
                  <div style={{color:K.tx4,fontSize:11}}>
                    Açıklama: <strong style={{color:K.tx}}>{kul?.email}</strong>
                  </div>
                </div>
              </div>
            ):(
              <div style={{color:K.tx4,fontSize:13,marginBottom:14,padding:14,
                background:K.bg3,borderRadius:10}}>
                IBAN henüz girilmemiş. Ayarlardan ekleyin.
              </div>
            )}
            <button onClick={()=>{
              const a=getA();
              const ny={id:Date.now(),ad:kul?.ad||"",email:kul?.email||"",
                tutar:odePlan.tutar||0,plan:odePlan.ad,
      ...
      {adModal&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.92)",
          display:"flex",alignItems:"center",justifyContent:"center",zIndex:9000}}>
          <div style={{background:K.card,borderRadius:18,padding:26,width:300,
            border:"1px solid "+K.bdr3,boxShadow:"0 24px 64px rgba(0,0,0,0.8)"}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:16}}>
            ...
            <div style={{display:"flex",gap:9}}>
              ...
              <button onClick={admGiris}
                style={{flex:1,padding:10,
                  background:"linear-gradient(135deg,"+K.g2+","+K.t2+")",
                  color:"#fff",border:"none",borderRadius:9,cursor:"pointer",fontWeight:700}}>
                Giriş
              </button>
            </div>
          </div>
